...
let rec eval_cdecl1 l env (CDecl1(name,expr,command)) =(*CDecl1を評価するための補助関数*)
  ...
  let rec eval_cletand l firstenv env (CLetAnd(name,expr,command)) =(*CLetAndを評価するための補助関数*)
  let nvar = eval_expr env expr in
  let nenv = extend name nvar env in
  match command with
  | CDecl(n,exp) -> 
    let nnvar =  eval_expr firstenv exp in
    (l@[("val "^name,nvar);("val "^n,nnvar)],extend n nnvar nenv)
  | CLet(n,e1,e2) -> 
      let nnvar =  eval_expr firstenv e1 in
      let nnenv = extend n nnvar nenv in
      ([("-",eval_expr nnenv e2)],firstenv)
  | CLetAnd(n,exp,ncom) ->
    eval_cletand (l@[("val "^name,nvar)]) firstenv nenv (CLetAnd(n,exp,ncom))
  | _ -> assert false(*絶対に到達しない状態*)

let eval_command env c =(*出力する変数名と値の組のリストと変更後の環境の組を返す*)
  match c with
  ...
  |CLetAnd(name,expr,command) ->eval_cletand [] env env (CLetAnd(name,expr,command))
  | _ ->failwith "CLet"
