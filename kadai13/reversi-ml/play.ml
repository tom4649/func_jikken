...
let fin_eval board color =
  ...
  let y = count board (opposite_color color) in
  x-y
...
let depth_read = 2(*時間の制約のため3手先まで読む*)
...
else if num< 40 then num_opt_self board color - 60*(!n_opt)+2*(evaluate board color)+(2*(count board color)-num) 
...
let play board color =
  ...
  if (n=5) then 
    if board.(4).(3) = black || board.(3).(4) =black then Mv (3,3)
    else Mv (6,6)
  else
