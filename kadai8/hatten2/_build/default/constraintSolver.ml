...
type ty =
  ...
  | TyFun of ty * ty
  | TyVar of tyvar
  | TyPair of mypair*mypair
and  mypair ={id:int;t:ty}
...
let id_ref = ref 0
let get_pair t0 = let n = !id_ref in id_ref:=n+1;{id=n;t=t0}
let rec lookup a l =
  match l with
  |[] -> raise TyError
...
let rec ty_subst sub a =(*型変数の代入後の値を返し、型変数が型代入に登録されていなければ例外TyErrorを返す関数*)
  ...
  | TyPair(a1,a2) -> 
    if a1.id=a2.id then let x = get_pair (ty_subst sub a1.t) in TyPair(x,x) else
    TyPair(get_pair (ty_subst sub a1.t),get_pair (ty_subst sub a2.t))
...
let rec occur_check a t=(*型変数aが型tの自由変数に含まれなければ真を返す関数*)
 ...
 | TyVar x -> a<>x
 |TyPair(t1,t2) -> 
  if t1.id=t2.id then occur_check a t1.t else
  occur_check a t1.t && occur_check a t2.t
let rec unify const =
  match const with
  | [] -> []
  ...
  print_string "start"; flush stdout;
    let rec get_fv_sub t0 ans =
      ...
      | TyBool -> ans
      | TyInt-> ans
      | TyFun(t1,t2) ->  get_fv_sub t2 (get_fv_sub t1 ans)
      | TyPair(t1,t2)->   print_string "pair"; flush stdout;
        if t1.id=t2.id then 
          (print_string "true"; flush stdout;
          get_fv_sub t1.t ans )
        else
          (print_string "false"; flush stdout;
        get_fv_sub t2.t (get_fv_sub t1.t ans))in
      get_fv_sub t []
...
print_int (!counter);flush stdout;
    let fv = get_type_vars t in print_string "var"; flush stdout;
    let fv_env = get_var_env env in print_string "env"; flush stdout;
...
let rec print_type t = match t with
  ...
  | TyPair(t1,t2) -> print_type t1.t;print_string"*";print_type t2.t
