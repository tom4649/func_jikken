let print_pair_flag_type =0(*ペアの出力を切り替えるフラグ*)
...
type ty =
  ...
  | TyFun of ty * ty
  | TyVar of tyvar
  | TyPair of mypair*mypair(*pair専用のレコード型を用意*)
...
let id_ref = ref 0(*mypairのid用の参照*)
let get_pair t0 = let n = !id_ref in id_ref:=n+1;{id=n;t=t0}(*pair用のレコード型を返す関数*)
let rec lookup a l =
  match l with
  |[] -> raise TyError
...
let rec print_type t = match t with
  ...
  | TyPair(t1,t2) -> 
    if print_pair_flag_type=0 then (print_string "pair")(*この課題の入力の場合出力が終わらないので省略*)
    else (print_type t1.t;print_string"*";print_type t2.t)
