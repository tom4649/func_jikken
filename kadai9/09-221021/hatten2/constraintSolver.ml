...
let rec ty_subst sub a =(*型変数の代入後の値を返し、型変数が型代入に登録されていなければ例外TyErrorを返す関数*)
  ...
  | TyPair(a1,a2) -> 
    TyPair(ty_subst sub a1,ty_subst sub a2)
  | Tylist t ->
    Tylist(ty_subst sub t)
  |TyExcep a ->TyExcep a
  |DefExcep -> DefExcep
...
let rec occur_check a t=(*型変数aが型tの自由変数に含まれなければ真を返す関数*)
 ...
 | TyVar x -> a<>x
 |TyPair(t1,t2) -> 
  occur_check a t1 && occur_check a t2
 |Tylist t -> occur_check a t
 |TyExcep _->true
 |DefExcep -> true
let rec unify const =
  match const with
  ...
  | TyExcep r -> if !r = DefExcep then r:= s2 else () ;unify ((!r,s2)::l)

  |DefExcep -> unify l 

  | TyFun(a1,b1) ->(match s2 with
  | DefExcep ->unify l
  | TyExcep r -> if !r = DefExcep then r:=s1 else () ;unify ((s1,!r)::l)
  ...
  |TyVar(a) -> (match s2 with
  | TyExcep r -> if !r = DefExcep then r:=s1 else () ;unify ((s1,!r)::l)
  | DefExcep ->unify l
  |_ ->
    if (occur_check a s2) then
    let substed = List.map (fun (t1,t2) -> (ty_subst [(a,s2)] t1,ty_subst [(a,s2)] t2)) l in compose (unify substed) [(a,s2)]
  else raise TyError)

  | TyPair(a1,b1) -> (match s2 with
  | DefExcep ->unify l
  | TyExcep r -> if !r = DefExcep then r:=s1 else () ;unify ((s1,!r)::l)
  | TyPair(a2,b2) -> unify([(a1,b1);(a2,b2)]@l)
  | TyVar a -> if (occur_check a s1) then
    let substed = List.map (fun (t1,t2) -> (ty_subst [(a,s1)] t1,ty_subst [(a,s1)] t2)) l in compose (unify substed) [(a,s1)]
  else raise TyError
  |_ -> raise TyError)

  |Tylist t ->(match s2 with
  | DefExcep ->unify l
  | TyExcep r -> if !r = DefExcep then r:=s1 else () ;unify ((s1,!r)::l)
  | Tylist s -> unify((t,s)::l)
  | TyVar a -> if (occur_check a s1) then
    let substed = List.map (fun (t1,t2) -> (ty_subst [(a,s1)] t1,ty_subst [(a,s1)] t2)) l in compose (unify substed) [(a,s1)]
  else raise TyError
  |_ -> raise TyError)
  ...
  let get_type_vars t =(*型に含まれる型変数のリストを返す関数*)
    let rec get_fv_sub t0 ans =
      ...
      | TyPair(t1,t2)->   
        get_fv_sub t2(get_fv_sub t1 ans)
      |Tylist s -> get_fv_sub s ans
      | TyExcep _-> ans
      | DefExcep -> ans
    in
      get_fv_sub t []
...
let rec print_type t = match t with
  ...
  |Tylist s -> print_type s;print_string" list"
  | TyExcep r-> print_type !r
  | DefExcep -> print_string ("()")
