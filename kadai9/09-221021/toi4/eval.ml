...
let rec eval_expr env e =
  ...
  | EVar x ->
      (try
         let Thunk(e0,env')=lookup x env in
         eval_expr env' e0
       with
  ...
  |EApp (e1,e2) ->
    let v1 = eval_expr env e1 in
    (match v1 with
    | VFun(x,e,env') -> eval_expr ((x,Thunk(e2,env))::env') e
    | VRecFun(f,x,e,oenv) ->
      let rec oenv'=((f,Thunk(EFun(x,e),oenv')))::oenv in
      let env' = (x,Thunk(e2,env))::oenv' in
      eval_expr env' e
    | _ -> raise EvalErr)
  |ELetRec(f,x,e1,e2) ->
    let rec env' = ((f,Thunk(EFun(x,e1),env')))::env in
...
let eval_command env c =
  ...
  | CDecl (name ,expr) -> ("val "^name,(name,Thunk(expr,env))::env,eval_expr env expr)
  | CRecDecl (f,x,e) -> 
    let rec env' = ((f,Thunk(EFun(x,e),env'))::env) in
